...
$rid=intval($_GET['rid']);
$fine=$_POST['fine'];
$getlabel=$_POST['isbnNum'];
$rstatus=1;
$sql="UPDATE `tblbooks` SET `Qunatity` = Qunatity + 1 WHERE `tblbooks`.`ISBNNumber` = :bookid";
$query = $dbh->prepare($sql);
$query->bindParam(':bookid',$getlabel,PDO::PARAM_STR);
$query->execute();
...
<div class="form-group">
<label>ISBN :</label>
<input type="hidden"   name="isbnNum" value="<?php echo $result->ISBNNumber;?>"  />
...
<?php if($result->retdate=="")
                                            {
                                                echo htmlentities("Not Return Yet");
                                            } else {


                                            echo htmlentities($result->retdate);
}
                                            ?>
</div>
...
<input class="form-control" type="text" name="fine" id="fine" value="<?php echo $result->fine ?>" required <?php if($result->RetrunStatus==1){ echo disabled; } ?>/>


</div>
 <?php if($result->RetrunStatus==0 || $result->RetrunStatus==2){?>
